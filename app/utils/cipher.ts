...
export const encodeMessage = (message: string): string => {
    return AES.encrypt(message, process.env.SECRET).toString()
}
export const decodeMessage = (messageEncoded: string): string => {
    const bytes = AES.decrypt(messageEncoded.toString(), process.env.SECRET)
    return bytes.toString(enc.Utf8)
}
export const encodePassword = (password: string): string => {
    const passwordWithSalt = password.concat(process.env.SALT)
